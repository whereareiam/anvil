...
class ForwardingPlannerTest {
	...
	void negotiatesDeterministicGroupsWithoutProvisioningOrCredentials() {
		...
		var forwarding = plan.stream().filter(group -> group.processes().contains("proxy")).findFirst().orElseThrow();
		assertEquals(ForwardingMode.MODERN, forwarding.mode());
		assertEquals(Set.of("proxy", "server"), forwarding.processes());
		assertEquals(ForwardingMode.NONE, plan.stream().filter(group -> group.processes().contains("direct"))
				.findFirst().orElseThrow().mode());
		assertEquals(plan, planner.plan(scenario, providers));
	...
	void negotiatesOneModeForOverlappingProxyRoutes() {
		...
		assertEquals(4, plan.getFirst().processes().size());
		assertTrue(plan.stream().allMatch(config -> config.mode() == ForwardingMode.LEGACY));
	}
	...
	void rejectsIncompatibleModesBeforeProvisioning() {
		...
		assertThrows(ScenarioValidationException.class, () -> planner.plan(scenario, providers));
	}
