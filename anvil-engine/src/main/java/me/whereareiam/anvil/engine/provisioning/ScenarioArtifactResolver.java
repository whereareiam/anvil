...
public final class ScenarioArtifactResolver {
	...
	private final AgentArtifactLocator agents;

	public ScenarioArtifactResolver(@NotNull Map<String, Path> artifacts, @NotNull AgentArtifactLocator agents) {
	...
	public @NotNull WorkspacePlan installAgent(
		...
		return workspace.toBuilder()
				.asset(WorkspaceAsset.builder()
						...
						.source(AssetSource.path(agents.locate(agent.getEntrypointClassName())))
	...
	private Path requireArtifact(String reference) {
		...
		if (artifact == null)
			throw new AnvilException("No artifact named '" + reference + "' was supplied. Available: "
					+ artifacts.keySet());
		if (!Files.isRegularFile(artifact))
			throw new AnvilException("Artifact '" + reference + "' does not exist: " + artifact);
