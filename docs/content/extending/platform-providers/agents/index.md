...
Start the common agent server during the platform plugin's startup with
`AgentServerProvider.discover().start(platformAgent, logConsumer)` and close the returned
`AgentServer` during shutdown. The engine supplies per-run authentication and execution-specific
...
accept; proxies declare the modes they send. The engine negotiates a mode shared by every member
