...
class McProtocolWorkerTest {
	...
	void dispatchesPlayerLifecycleAndExternalOperationsUntilShutdown() throws Exception {
		...
		var binding = ServiceLoader.load(ProtocolWorkerBinding.class).stream().map(ServiceLoader.Provider::get)
				.filter(value -> value.family().equals("mcprotocol-2026")).findFirst().orElseThrow();
		try (var worker = new McProtocolWorker(registry, writer, binding)) {
