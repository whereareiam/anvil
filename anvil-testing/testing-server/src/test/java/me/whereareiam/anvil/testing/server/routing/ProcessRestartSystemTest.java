...
class ProcessRestartSystemTest {
	...
	private void verify(AnvilScenario original) throws Exception {
		...
		try (AnvilEngine engine = new AnvilEngine(EngineOptions.builder().eulaAccepted(true).build());
			...
			var before = context.process(scenario.getEntrypoint());
			Files.writeString(before.workDirectory().resolve("restart-marker"), "preserved");
			var after = context.restart(before.name());
			...
			if (!scenario.getProxies().isEmpty()) {
				session.disconnect();
				session.disconnected();
				var proxy = context.proxy("proxy");
				context.restart(destination);
				assertSame(proxy, context.proxy("proxy"));
